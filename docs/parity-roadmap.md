...
| **Provider response caching** ✅ | M | medium | ✅ DONE — `CachingLlmProvider`/`CachingEmbeddingProvider` decorators (PR #5) **plus** a pluggable `CacheBackend` trait with `InMemoryCacheBackend` (default) and **`DiskCacheBackend`** (PR #21): JSON-file-per-entry persistence keyed by a fixed-seed `DefaultHasher` (full key stored in-file + verified on read → collisions degrade to a miss, never a wrong value), all I/O best-effort (cache failure never breaks eval). Re-running an eval over the same inputs serves cached responses instead of re-calling the model — faithful analog of Python's `DiskCacheBackend` minus the `diskcache` dep. Deterministic (cross-instance persistence test, no live gate). |
