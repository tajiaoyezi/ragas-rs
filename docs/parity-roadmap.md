...
**Phase 6 ✅ (parity-complete with an honest deferred tail — closed 2026-06-07)** — full testset
generation stack: 7 LLM extractors (`LlmExtractor`/`LlmExtractorKind`), transforms engine
(`apply_transforms` / `default_transforms`), embedding/cosine/overlap relationship builders,
`CustomNodeFilter`, `split_by_headlines`, KG persona generation, all three named synthesizers
(`SingleHopSpecific`, `MultiHopSpecific`, `MultiHopAbstract`) + `TestsetGenerator` orchestrator.
Live-verified end-to-end (gate **DT**, ~153s): raw text → `default_transforms` → `TestsetGenerator`
→ tagged test set vs DeepSeek+SiliconFlow (2026-06-07); **12 Phase-6 live gates**, 136 offline
testset tests. Deferred tail: Leiden clustering, `RegexBasedExtractor` (unused by default pipeline),
tiktoken bin boundaries / NumPy RNG shuffles (documented non-goals).
...
## Phase 6 — Testset generation stack ✅ DONE (parity-complete; honest deferred tail)
...
> ✅ **Phase 6 closed 2026-06-07.** The remaining items are **deferred / N/A**, not unfinished work
> — documenting them honestly rather than adding dead code:
> - **Leiden `find_indirect_clusters`** — deliberately skipped; the portable
>   `find_n_indirect_clusters` (seeded-DFS) covers the need and is wired into
>   `MultiHopAbstractSynthesizer` (gate **MHA** passed).
> - **`RegexBasedExtractor`** — deferred; unused by the default generation pipeline; would need a
>   feature-gated regex engine the default build deliberately excludes.
> - **tiktoken bin boundaries / NumPy RNG shuffles** — documented non-goals; char-budget branching
>   and deterministic ordering are the faithful substitutes.
> - **Standalone `generate_scenarios` / `generate_sample` Python methods** — N/A by Rust design;
>   scenario prep is split across `prepare_*_scenarios` + synthesizer `generate` methods (functional
>   equivalent; gates **SH**, **SHS**, **MHS**, **MHA**, **TG** passed).
>
> Everything else in Phase 6 is done: 7 LLM extractors, transforms engine (`apply_transforms` /
> `default_transforms`), embedding/cosine/overlap builders, `CustomNodeFilter`, `split_by_headlines`,
> KG persona generation, all three named synthesizers, `TestsetGenerator` orchestrator, and the
> raw-text→testset end-to-end path (gate **DT**). **Phase 6 ✅.**

| Item | Effort | Value | Approach |
|---|---|---|---|
| **KnowledgeGraph widening + cluster/relationship queries** ✅ | M | high | ✅ DONE 2026-06-07 — `find_n_indirect_clusters(graph, n, depth_limit, bidirectional, relationship_filter)`: deterministic seeded-DFS port (path-clusters of 2..depth_limit nodes, per-start collection, round-robin dedup with superset-evicts-subset). RNG `random.shuffle` of start nodes dropped (non-goal) → sorted-id order; `bidirectional` is a caller flag (this crate's `GraphEdge` has no per-edge flag — similarity clusters pass `true`). Errors on depth_limit<2 / n<1 / no-match (Python's 3 ValueErrors). 7 hand-computed offline tests. Earlier: `find_two_nodes_single_rel` analog inlined as `entity_overlap_clusters` (PR #14). Wired into `MultiHopAbstractSynthesizer` (gate **MHA**). **Deferred:** Leiden `find_indirect_clusters` (graph-algorithm dep; not needed for the portable path). |
...
| **Splitters + relationship builders + embedding/regex extractors + node filters** ✅ | M | high | ✅ DONE 2026-06-07 — `EmbeddingExtractor` (embed node text → `GraphProperty::Vector`) + `build_cosine_relationships` (faithful `CosineSimilarityBuilder`: pairwise cosine over embedded nodes → `cosine_similarity` edges ≥ threshold; live-verified vs SiliconFlow, gate **CB**) + `build_overlap_relationships` (faithful `OverlapScoreBuilder`: per-pair fuzzy Jaro-Winkler entity matching with the top-5% "noisy item" exclusion → directed `entities_overlap` edges; deterministic, hand-computed tests) + `CustomNodeFilter` (LLM scores each chunk 1–5 against its parent doc's `summary` + rubric, drops `score <= min_score`; live-verified vs DeepSeek, gate **NF**) + `split_by_headlines` (faithful `HeadlineSplitter`: cuts a document's `text` at its `headlines`, merges small / word-splits large pieces via `adjust_headline_chunks` — char-budget substitutes for tiktoken — into `chunk` nodes linked by `child`+`next` edges) + `extract_markdown_headings` (deterministic `markdown_headings_extractor` analog, no regex dep). **Deferred:** `RegexBasedExtractor` (unused by default pipeline; offer feature-gate if wanted). |
| **Persona generation from KG + scenario model** ✅ | M | high | ✅ DONE 2026-06-07 — `generate_personas_from_kg` (faithful port of `persona.py`: filter nodes with `summary`+`summary_embedding`, greedy cosine>0.75 clustering, longest-summary representative, LLM-generate `Persona{name, role_description}`; skips `np.random` padding — documented). Live-verified vs DeepSeek (gate **PG**). Scenario prep landed with the synthesizers: `prepare_single_hop_scenarios` (gate **SH**), `prepare_multi_hop_specific_scenarios`, `prepare_multi_hop_abstract_scenarios` — functional equivalent of Python's `generate_scenarios`/`generate_sample` split. |
| **Three named synthesizers + TestsetGenerator orchestrator** ✅ | L | high | ✅ DONE 2026-06-07 — **All three synthesizers AND the `TestsetGenerator` orchestrator complete end-to-end.** `TestsetGenerator`: `new(llm, kg)` + `with_personas`/`with_llm_context`/`with_query_distribution` + `generate(testset_size, num_personas)` — derives personas (via `generate_personas_from_kg`) or accepts a list, picks `default_query_distribution` (includes only synthesizers whose structure is present — uniform weights), splits via `calculate_split_values` (`ceil(n·w)`), runs each synthesizer, merges into one `EvaluationDataset` tagged with `synthesizer_name`. Live-verified vs DeepSeek (gate **TG**, ~72s). **SingleHopSpecific** (gate **SHS**), **MultiHopSpecific** (gate **MHS**), **MultiHopAbstract** (gate **MHA**). **Documented divergence:** Python leaves `response`/`retrieved_contexts` for the system-under-test; this crate mirrors answer/context into them because `EvaluationDataset` requires them non-empty. |
