# Task 8.2 - output-parser

**Status**: Done
**Phase**: 8
**PRD**: docs/prds/ragas-rs-complete-refactor.prd.md

## 1. Background

This task is part of the complete Rust refactor of upstream ragas commit 298b682. It expands the previously completed MVP core toward full project coverage.

## 2. Goal

JSON/schema parser、repair strategy、malformed output diagnostics

## 3. Scope And Out-of-Scope

**In scope**:
- Rust module area: `src/prompts/mod.rs` and public exports in `src/lib.rs`.
- Behavior listed in §6 acceptance criteria.
- Unit tests and, where applicable, parity fixtures for upstream ragas semantics.

**Out of scope**:
- Unrelated phases from the complete refactor matrix.
- Hidden Python runtime dependency or pyo3 bridge.
- Marking parity complete without explicit fixture evidence.

## 4. Actors

- Rust caller using ragas-rs.
- Evaluation framework maintainer tracking Python ragas parity.

## 5. Behavior Contract

### 5.1 Required Reading

- docs/prds/ragas-rs-complete-refactor.prd.md
- docs/specs/ragas-complete-refactor-breakdown.md
- test/features/output-parser.feature

### 5.2 Imports

Use existing public crate exports unless this task explicitly creates a new module boundary.

### 5.3 Function Signatures

Function signatures are owned by this task's RED tests and must be added before GREEN implementation.

## 6. Acceptance Criteria

- **AC1**: Parser extracts typed JSON scores and reasons
- **AC2**: Malformed judge output returns parse diagnostics with raw excerpt
- **AC3**: Repair strategy is explicit and testable

## 7. Traceability

| AC | Scenario | Test ID | Status |
|---|---|---|---|
| AC1 | SCEN-8.2.1 | TEST-8.2.1 | Done |
| AC2 | SCEN-8.2.2 | TEST-8.2.2 | Done |
| AC3 | SCEN-8.2.3 | TEST-8.2.3 | Done |

## 8. Risks

- Upstream Python semantics may not map one-to-one to Rust types.
- Optional external integrations must not leak into the default dependency set.

## 9. Verification Plan

- install
- typecheck
- unit-test
- build

## 10. Completion Notes

- **完成日期**：2026-05-31
- **改动文件**：
  - `src/prompts/mod.rs`（新增 `JudgeOutputParser`、`ParsedJudgeOutput`、`OutputParseDiagnostic`、`RepairStrategy` 和 task 8.2 unit tests）
  - `src/lib.rs`（导出 output parser API）
  - `docs/specs/tasks/task-8.2-output-parser.md`（Status/traceability/Completion Notes 回填）
- **commit 列表**：
  - `8fa9a01` docs(spec): task-8.2 Ready
  - `938d0f6` docs(spec): task-8.2 进入实施
  - `81a4ccc` test(prompts): 加 task-8.2 RED 测试
  - `9a0369e` feat(prompts): 实现 task-8.2 output parser
- **§9 Verification 结果**：
  - install: pass (`cargo build`)
  - typecheck: pass (`cargo check`)
  - unit-test: 46 passed / 0 failed (`cargo test`)
  - build: pass (`cargo build`)
  - note: project helper `s2v_verify_full` still cannot parse lowercase generated §9 keys, so adapter commands were executed directly and recorded here.
- **剩余风险 / 未做项**：本 task 实现 score/reason judge JSON parser 和简单 object extraction repair；复杂 schema validation、LLM self-repair prompt 和 multimodal parser 留给后续 metric/parity tasks。
- **下游 task 影响**：task 9/10 metric judge flows can parse structured scores through `JudgeOutputParser`; task 8.3 can reuse `OutputParseDiagnostic` for multimodal prompt/parser diagnostics.
